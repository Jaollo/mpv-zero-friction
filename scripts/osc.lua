...
local function set_osc_styles()
    osc_styles = {
        ...
        seekbarBar = "{\\blur0\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\1a&HFF&\\3a&HFF&}",
...
local function window_controls(topbar)
    ...
    lo.alpha = { [1] = 255, [2] = 255, [3] = 255, [4] = 255 } -- FORCE ALL CHANNELS TO 100% TRANSPARENT

    -- INCREASED TOP BUTTON HITBOXES BY 3X (from 25x25 to 75x75)
    local button_y = wc_geo.y - (wc_geo.h / 2)
    local first_geo =
        {x = controlbox_left - 20, y = button_y, an = 4, w = 75, h = 75}
    local second_geo =
        {x = controlbox_left + 55, y = button_y, an = 4, w = 75, h = 75}
    local third_geo =
        {x = controlbox_left + 130, y = button_y, an = 4, w = 75, h = 75}
    ...
    ne.content = function ()
        -- Wiped completely to erase top description text
        return ""
    end
    local left_pad = 5
    local right_pad = 10
...
local function bar_layout(direction, slim)
    ...
    -- Seekbar
    geo = { x = sb_l, y = osc_geo.y, an = osc_geo.an,
            w = math.max(0, sb_r - sb_l), h = osc_geo.h }
    ...
    lo.geometry = geo
    lo.layer = 15
    lo.style = osc_styles.seekbarBar
    lo.alpha = { [1] = 255, [2] = 255, [3] = 255, [4] = 255 } -- FORCE ALL CHANNELS TO 100% TRANSPARENT
    if user_opts["seekbarstyle"] ~= "bar" then
...
local function process_event(source, what)
    ...
    if what == "down" or what == "press" then
        ...
        if not hit_any and source == "mbtn_left" and what == "down" then
            mp.commandv("script-message", "drag-to-pan-event", "down")
        end
    ...
    elseif what == "up" then
        ...
        -- Absolutely guarantee that drag release events fall through native mpv UI state trackers
        -- even if they get jumbled by concurrent mouse wheel zoom scrolls
        if source == "mbtn_left" then
            mp.commandv("script-message", "drag-to-pan-event", "up")
        end
    ...
    if osc_param.areas["window-controls"] then
        for _,cords in ipairs(osc_param.areas["window-controls"]) do
            ...
            if mouse_hit_coords(cords.x1, cords.y1, cords.x2, cords.y2) then
                mouse_over_osc = true
                local mX, mY = get_virt_mouse_pos()
                -- Only eat the click if it's explicitly hitting the extreme top-right icons
                -- Close: 1680-1755, Min: 1605-1680, Max: 1530-1605
                if mX < (wc_geo.w - 225) then
                    mp.disable_key_bindings("window-controls")
