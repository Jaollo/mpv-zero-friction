...
| OSC's `mouse_leave` sends `drag-to-pan-event "cancel"` | `osc.lua` `mouse_leave()` | The OSC's Tier 3 `mouse_leave` handler consumes the event before drag-to-pan's Tier 2 kill switch can fire. Without this cancel, drags survive mouse-leave and `mouse_in_window` becomes false, causing `get_virt_mouse_pos()` to return -1,-1 on subsequent clicks — seekbar stops responding. |
| `process_event` sets `mouse_in_window = true` on button events | `osc.lua` line 2300 | Forced bindings (drag-to-pan) steal `mouse_move` from the showhide group, preventing `mouse_in_window` from being refreshed. If a button event fires from the "input" group, the mouse IS in the window. Without this fix, `get_virt_mouse_pos()` returns -1,-1 after drags, and `mouse_hit()` fails for all elements — seeking stops working until the user moves the mouse. |
